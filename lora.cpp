...
static byte calcCRC(byte* buf, uint8_t len, bool useSyncID) {
    ...
    if (useSyncID)
        crc ^= currentSyncID[0] ^ currentSyncID[1] ^ currentSyncID[2];
    return crc;
}
...
static uint32_t smartJitter(uint32_t pktDurationMs = 500) {
    ...
    for (uint8_t offset = 0; offset < 60; offset++) {
        ...
        for (uint8_t u = 0; u < MAX_UNITS; u++) {
            ...
            if (testSec == ws % 60) { occupied = true; break; }
            // Ultimele 500ms inainte de fereastra?
            if (testSec == (ws == 0 ? 59 : ws - 1) % 60 &&
                offset == 0 && msInSec >= 500) { occupied = true; break; }
                // Pachetul se termina in fereastra altcuiva?
                if (pktEndSec == ws % 60) { occupied = true; break; }
        }

        if (!occupied) {
            uint32_t msToWait = (offset * 1000);
            if (msToWait >= msInSec) msToWait -= msInSec;
            else msToWait = 0;
...
static void buildHeartbeat(
    ...
    txBuf[13] = ((mode   & 0x0F) << 4) | (status & 0x0F);
    txBuf[14] = ((batLvl & 0x0F) << 4) | (pendingEventType & 0x0F);
    txBuf[15] = calcCRC(txBuf, 15, true);
    txLen     = 16;
    ...
    static void buildSync() {
        ...
        txBuf[3] = PKT_SYNC;
        txBuf[4] = UNIT_ID;
        txBuf[5] = currentSyncID[0]; txBuf[6] = currentSyncID[1]; txBuf[7] = currentSyncID[2];
    ...
    static void buildEpochSync() {
        memset(txBuf, 0, 6);
        txBuf[0] = NETWORK_ID[0]; txBuf[1] = NETWORK_ID[1]; txBuf[2] = NETWORK_ID[2];
        txBuf[3] = PKT_EPOCH_SYNC;
        txBuf[4] = UNIT_ID;
        txBuf[5] = calcCRC(txBuf, 5, true);
        txLen     = 6;
    ...
    static void onTransmitDone(
        ...
        if (txPktType == PKT_HEARTBEAT) {
            Serial.print("[LORA] Heartbeat transmis. Unit "); Serial.println(UNIT_ID);
            if (pendingEventType != EVT_NONE) {
                Serial.print("[LORA] Piggyback livrat: "); Serial.println(pendingEventType);
            ...
            Serial.print("[LORA] Urgent trimis: "); Serial.println(s_urgentEvent);
            if (pendingEventType == s_urgentEvent) {
                pendingEventType = EVT_NONE;
                pendingEventTeam = 0;
            }
        ...
        // ============================================================
        static void processPacket(byte* buf, uint8_t len,
                                  int32_t liveScore[4], uint16_t teamKills[4],
                                  bool& isTimeOut, bool& isGameTimerRunning,
                                  uint32_t& gameTimeLeftSeconds) {

            if (buf[0] != NETWORK_ID[0] ||
                buf[1] != NETWORK_ID[1] ||
                buf[2] != NETWORK_ID[2]) return;
            ...
            Serial.print(" Unit "); Serial.println(sender);

            uint32_t now = millis();
            lastSeenTime[sender - 1] = now;

            if (pktType == PKT_SYNC) {
                currentSyncID[0] = buf[5]; currentSyncID[1] = buf[6];
                currentSyncID[2] = buf[7]; currentSyncID[3] = '\0';
                ...
                Serial.print("[LORA] SYNC primit. SyncID: "); Serial.println(currentSyncID);

            } else if (pktType == PKT_START) {
                if (!isGameTimerRunning) {
                    uint32_t timeLeft   = ((uint32_t)buf[5] << 24) |
                    ((uint32_t)buf[6] << 16) |
                    ((uint32_t)buf[7] << 8)  |
                    buf[8];
                    isGameTimerRunning  = true;
                ...
                uint8_t mode   = (buf[13] >> 4) & 0x0F;
                uint8_t status =  buf[13]        & 0x0F;
                ...
                globalBattery[sender-1] = (buf[14] >> 4) & 0x0F;
                uint8_t piggy           =  buf[14]        & 0x0F;
                ...
                Serial.print("[LORA] Urgent "); Serial.print(eType);
                Serial.print(" team "); Serial.print(teamId);
                Serial.print(" Unit "); Serial.println(sender);
                ...
                if (!isTimeOut) {
                    isTimeOut          = true;
                    isGameTimerRunning = false;
                    Serial.print("[LORA] CONQUEST! Winner: "); Serial.println(buf[5]);
                    tone(PIN_BUZZER, 1800, 600);
                                  ...
                                  void loraInit() {
                                      ...
                                      Serial.print("[LORA] Init. SyncID izolat: "); Serial.println(currentSyncID);
                                  }
                                  ...
                                  void loraUpdate(
                                      ...
                                      while (LoRaSerial.available() > 0 &&
                                          ...
                                          if (LoRaSerial.available() >= 6) {
                                              ...
                                              if      (pktType == PKT_SYNC)       pktLen = 44;
                                              else if (pktType == PKT_HEARTBEAT)  pktLen = 16;
                                              else if (pktType == PKT_START)      pktLen = 10;
                                              else if (pktType == PKT_EPOCH_SYNC) pktLen = 6;
                                              else if (pktType == PKT_RESTART)    pktLen = 6;
                                              else if (pktType == PKT_CONQUEST)   pktLen = 7;

                                              uint8_t remaining = pktLen - 4;

                                              if (LoRaSerial.available() >= remaining) {
                                                  byte buf[44] = {0};
                                                  buf[0] = header[0]; buf[1] = header[1];
                                                  buf[2] = header[2]; buf[3] = header[3];
                                                  LoRaSerial.readBytes(buf + 4, remaining);

                                                  processPacket(buf, pktLen,
                                                                liveScore, teamKills,
                                          ...
                                          // 3. JITTER PENDING
                                          // --------------------------------------------------------
                                          if (jitterPending && txState == TX_IDLE &&
                                              now - jitterStart >= jitterDelay) {

                                              // Pentru sync, reverificam fereastra la momentul transmisiei
                                              ...
                                              jitterPending = false;
                                          if      (jitterPktType == PKT_SYNC)   buildSync();
                                          else if (jitterPktType == PKT_URGENT) buildUrgent(s_urgentEvent, s_urgentTeam);
                                          startTransmit();
                                              }
                                              ...
                                              // 4. RETRANSMISIE
                                              // --------------------------------------------------------
                                              if (retxCount > 0 && retxCount <= retxMax &&
                                                  txState == TX_IDLE && now - retxStart >= retxDelay) {
                                      ...
                                      void loraSendSync(
                                          ...
                                          for (int i = 0; i < 3; i++) currentSyncID[i] = charset[random(0, 62)];
                                          currentSyncID[3] = '\0';
                                          Serial.print("[LORA] Nou SyncID: "); Serial.println(currentSyncID);

                                          s_gsTimeLimit = gsTimeLimit; s_gsBonus = gsBonus; s_gsWinCond = gsWinCond;
                                          s_bsTimerIdx  = bsTimerIdx;  s_bsCooldownIdx = bsCooldownIdx;
                                          s_bsExpPtsIdx = bsExpPtsIdx; s_bsDefPtsIdx   = bsDefPtsIdx;
                                          s_rsTimeIdx   = rsTimeIdx;   s_rsPenaltyIdx  = rsPenaltyIdx;
                                          for (uint8_t i = 0; i < 4; i++) s_rsLimitIdx[i] = rsLimitIdx[i];
                                          s_isRunning    = isRunning; s_isTimeOut = isOver;
                                          s_gameTimeLeft = gameTimeLeft;
                                          for (uint8_t i = 0; i < 4; i++) { s_scores[i] = scores[i]; s_kills[i] = kills[i]; }
                                          ...
                                          int32_t loraGetNetworkScore(uint8_t teamIdx, int32_t localScore) {
                                              ...
                                              for (uint8_t u = 0; u < MAX_UNITS; u++) {
                                                  ...
                                                  if (globalScores[u][teamIdx] > 0)
                                                      total += globalScores[u][teamIdx];
                                              }
                                              return total;
                                          }
                                          ...
                                          void loraSendRestartBlocking() {
                                              buildRestart();
                                              // Trimitem de 2 ori, blocant, pentru ca urmeaza restart
